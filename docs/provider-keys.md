...
SourceFoundry can operate public feeds without a commercial search provider. When bounded discovery is useful, the current high-level source-feed API supports Tavily, Exa, and Serper.
...
Then set `provider` to `tavily`, `exa`, or `serper` on the discovery source. SourceFoundry chooses the official endpoint, applies a result cap, strips provider-specific response fields, and emits the same candidate contract.
