...
SourceFoundry can operate public feeds without a commercial search provider. When search discovery is useful, the current high-level source-feed API supports Tavily, Exa, and Serper.
...
Then set `provider` to `tavily`, `exa`, or `serper` on the discovery source. SourceFoundry chooses the official endpoint, applies a result cap, strips provider-specific response fields, and emits the same source-item contract.
...
Open source lets operators run SourceFoundry themselves and connect accounts they already control. It removes the need for SourceFoundry to pay for every self-hosted search call.
...
## Hosted SourceFoundry

A hosted service has two safe models:

1. SourceFoundry uses its own keys only for provider lanes covered by an agreement that permits the hosted customer use.
