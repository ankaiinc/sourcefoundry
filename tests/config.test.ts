...
describe('SourceFoundry runtime configuration', () => {
  ...
  it('publishes an HTTPS canonical endpoint in production', () => {
    expect(loadConfig({
      ...
      SOURCEFOUNDRY_PUBLIC_BASE_URL: 'https://sourcefoundry.4agents.fyi/',
    }).publicBaseUrl).toBe('https://sourcefoundry.4agents.fyi');

    expect(() => loadConfig({
      NODE_ENV: 'production',
      SOURCEFOUNDRY_DATABASE_URL: 'postgres://db',
      SOURCEFOUNDRY_RELEASE_SHA: 'a'.repeat(40),
      SOURCEFOUNDRY_PUBLIC_BASE_URL: 'http://sourcefoundry.4agents.fyi',
