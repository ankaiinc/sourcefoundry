...
const config = {
  publicBaseUrl: 'https://feedline.4agents.fyi',
...
describe('SourceFoundry agent discovery', () => {
  it('publishes a stable, canonical tool-discovery descriptor', () => {
    ...
    expect(descriptor).toMatchObject({
      schemaVersion: 1,
      release: 'a'.repeat(40),
      baseUrl: 'https://feedline.4agents.fyi',
      authentication: { type: 'http-bearer', environmentVariable: 'SOURCEFOUNDRY_API_TOKEN' },
      discovery: {
        openapi: 'https://feedline.4agents.fyi/openapi.json',
        agentGuide: 'https://feedline.4agents.fyi/agent.md',
        llms: 'https://feedline.4agents.fyi/llms.txt',
      },
    });
  });
  ...
  it('describes the full safe agent workflow in OpenAPI', () => {
    ...
    expect(document.servers[0]?.url).toBe('https://feedline.4agents.fyi');
