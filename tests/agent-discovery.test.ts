...
const config = {
  publicBaseUrl: 'https://sourcefoundry.4agents.fyi',
...
describe('SourceFoundry agent discovery', () => {
  it('publishes a stable, canonical tool-discovery descriptor', () => {
    ...
    expect(descriptor).toMatchObject({
      schemaVersion: 1,
      release: 'a'.repeat(40),
      baseUrl: 'https://sourcefoundry.4agents.fyi',
      ...
      discovery: {
        ...
        agentGuide: 'https://sourcefoundry.4agents.fyi/agent.md',
        llms: 'https://sourcefoundry.4agents.fyi/llms.txt',
      },
    });
  });
  ...
  it('describes the full safe agent workflow in OpenAPI', () => {
    ...
    expect(document.servers[0]?.url).toBe('https://sourcefoundry.4agents.fyi');
    expect(document.paths).toHaveProperty('/v1/tenants');
    expect(document.paths).toHaveProperty('/v1/agent-enrollments');
    expect(document.paths).toHaveProperty('/v1/discovery-options');
    expect(document.paths).toHaveProperty('/v1/discovery-plans/resolve');
